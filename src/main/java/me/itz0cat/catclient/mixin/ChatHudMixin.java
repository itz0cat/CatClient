...
import me.itz0cat.catclient.api.hook.ChatHudHook;
import me.itz0cat.catclient.api.hook.IChatHudExt;
...
public abstract class ChatHudMixin implements IChatHudExt {
    ...
    private Text compactchat$compactChatMessage(Text message, Text parameterMessage, MessageSignatureData data, MessageIndicator indicator) {
        if (!CatClient.modManager().getMod(GeneralSettings.class).stackChatMessages.isEnabled()) {
            return message;
        }
