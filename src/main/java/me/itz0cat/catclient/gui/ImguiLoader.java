...
import static me.itz0cat.catclient.gui.TextureLoader.loadImage;
import static me.itz0cat.catclient.gui.TextureLoader.loadTexture;
...
public class ImguiLoader {
    ...
    private static void initializeImGui() {
        ...
        try (InputStream is = ImguiLoader.class.getClassLoader().getResourceAsStream("assets/catclient/cattext.png")) {
            if (is != null) {
                BufferedImage image = loadImage(is);
                catText = loadTexture(image);
            }
        } catch (IOException ignored) {

        }

        try (InputStream is = ImguiLoader.class.getClassLoader().getResourceAsStream("assets/catclient/icon.png")) {
            if (is != null) {
                BufferedImage image = loadImage(is);
                catLogo = loadTexture(image);
            }
        } catch (IOException ignored) {
