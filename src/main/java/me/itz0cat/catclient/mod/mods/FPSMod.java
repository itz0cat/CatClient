...
public class FPSMod extends Mod implements Renderable {
    ...
    @Override
    public void render() {
        if(!CatClient.modManager().getMod("FPS").isEnabled()) {
            firstFrame = true;
            return;
        }
        ...
        String text;
        if(backgroundEnabled.isEnabled()) text = FPSHelper.getFPS() + " FPS";
        else text = "[" + FPSHelper.getFPS() + " FPS]";
