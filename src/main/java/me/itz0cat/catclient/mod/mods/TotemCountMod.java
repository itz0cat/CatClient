...
public class TotemCountMod extends Mod implements Renderable {
    ...
    public TotemCountMod() {
        super("Totem Count", "Shows your amount of totems.", "\uF654");
        toggleVisibility();
    }
    ...
    @Override
    public void render() {
        if(!CatClient.modManager().getMod("Totem Count").isEnabled()) {
            firstFrame = true;
            return;
        }
        ...
        String text;
        if(backgroundEnabled.isEnabled()) text = getArrows() + " totems";
        else text = "[" + getArrows() + " totems]";
