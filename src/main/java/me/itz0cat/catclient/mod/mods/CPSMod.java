...
public class CPSMod extends Mod implements Renderable {
    ...
    @Override
    public void render() {
        if(!CatClient.modManager().getMod("CPS").isEnabled()) {
            firstFrame = true;
            return;
        }
        ...
        String text = !rightClickEnabled.isEnabled() ?
                backgroundEnabled.isEnabled() ?
                        CPSHelper.getCPS(0) + " CPS" :
                        "[" + CPSHelper.getCPS(0) + " CPS]" :
                backgroundEnabled.isEnabled() ? CPSHelper.getCPS(0) + " | " + CPSHelper.getCPS(1) + " CPS" :
                        "[" + CPSHelper.getCPS(0) + " | " + CPSHelper.getCPS(1) + " CPS]";
