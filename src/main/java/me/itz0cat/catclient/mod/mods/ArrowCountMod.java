...
import me.itz0cat.catclient.api.helpers.ItemHelper;
...
public class ArrowCountMod extends Mod implements HudElement {
    ...
    public ArrowCountMod() {
        super("Arrow Count", "Shows your amount of arrows.", "\uF05B");
        this.category = Category.HUD;
        HudManager.getInstance().register(this);
    }

    private int getArrows() {
        return ItemHelper.getTotal(new ItemStack(Items.ARROW))
                + ItemHelper.getTotal(new ItemStack(Items.SPECTRAL_ARROW))
                + ItemHelper.getTotal(new ItemStack(Items.TIPPED_ARROW));
    }
    ...
    public void renderHud(DrawContext context, RenderTickCounter tickCounter) {
        ...
        int count = getArrows();
        String textStr = (backgroundEnabled.isEnabled() ? "" : "[") + count + " arrows" + (backgroundEnabled.isEnabled() ? "" : "]");
