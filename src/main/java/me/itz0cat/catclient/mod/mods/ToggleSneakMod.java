...
public class ToggleSneakMod extends Mod implements Renderable {
    ...
    public final NumberSetting width = new NumberSetting("Width", this, 200, 100, 250, 1);
    ...
    public ToggleSneakMod() {
        super("Toggle Sneak", "Allows you to toggle sneak.", "\uF183");
        toggleVisibility();
    }
    ...
    private long getPing() {
        return CatClient.mc.getCurrentServerEntry() == null ? -1 : CatClient.mc.getCurrentServerEntry().ping;
    }

    @Override
    public void render() {

        if(!CatClient.modManager().getMod("Toggle Sneak").isEnabled()) {
            firstFrame = true;
            return;
        }
        ...
        String text;
        if(backgroundEnabled.isEnabled()) text = "Sneak: " + (isSneaking() ? "Toggled" : "Vanilla");
        else text = "[Sneak: " + (isSneaking() ? "Toggled]" : "Vanilla]");
