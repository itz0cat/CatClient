...
public class ToggleSneakMod extends Mod implements HudElement {
    ...
    public void onKey(KeyPressEvent e) {
        if (mc.options != null && mc.options.sneakKey != null &&
                InputUtil.isKeyPressed(MinecraftClient.getInstance().getWindow(), mc.options.sneakKey.getDefaultKey().getCode())) {
            toggled = !toggled;
        }
    }
    ...
    public void renderHud(DrawContext context, RenderTickCounter tickCounter) {
        ...
        String status = isSneaking() ? "Toggled" : "Vanilla";
        String textStr = (backgroundEnabled.isEnabled() ? "" : "[") + "Sneak: " + status + (backgroundEnabled.isEnabled() ? "" : "]");
