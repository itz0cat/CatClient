...
public class ServerIPMod extends Mod implements Renderable {
    ...
    public ServerIPMod() {
        super("Server IP", "Shows your server's IP.", "\uF3C5");
        toggleVisibility();
    }
    ...
    @Override
    public void render() {
        if(!CatClient.modManager().getMod("Server IP").isEnabled()) {
            firstFrame = true;
            return;
        }
        ...
        String text;
        if(backgroundEnabled.isEnabled()) text = getAddress();
        else text = "[" + getAddress() + "]";
