...
public class ReachDisplayMod extends Mod implements Renderable {
    ...
    public void onAttack(AttackEntityEvent e) {
        ...
        if (mc.crosshairTarget.getType().equals(HitResult.Type.ENTITY)) {

            assert mc.player != null;
            Vec3d vec3 = mc.player.getCameraPosVec(1);
            blocks = mc.crosshairTarget.getPos().distanceTo(vec3);
        }
    }

    @Override
    public void render() {
        if(!CatClient.modManager().getMod("Reach Display").isEnabled()) {
            firstFrame = true;
            return;
        }
