...
public class ToggleSprintMod extends Mod implements Renderable {
    ...
    public void render() {
        ...
        if(!CatClient.modManager().getMod("Toggle Sprint").isEnabled()) {
            firstFrame = true;
            return;
        }
        ...
        String text;
        if(backgroundEnabled.isEnabled()) text = "Sprint: " + (isSprinting() ? "Toggled" : "Vanilla");
        else text = "[Sprint: " + (isSprinting() ? "Toggled]" : "Vanilla]");
